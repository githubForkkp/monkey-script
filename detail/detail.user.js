...
document.onreadystatechange = function(){
	...
	if (document.readyState == "complete") {
		...
		// 编辑宝贝
		var edit = matchNode('//span[@id = "J_EditItem"]').snapshotLength;
		// 宝贝价格
		...
		// 服务承诺
		var _service = matchNode('//dd[@class = "tb-serPromise tm-laysku-dd"]').snapshotLength;
		// 爱分享与收藏
...
//             success: function(d) {
//             	Beans = d;
//             	var dsr = "SM_368_dsr-"+config.itemDO.userId
//             	if (Beans[dsr] !== 0) {
//             		document.title=Beans[dsr].m+" "+Beans[dsr].s+" "+Beans[dsr].c;
//             	};
//             }
//         })
...
var wanrentuan = new ks.IO({
            ...
            success: function(d) {
                ...
                // 优惠对象,针对无sku的商品
                var def = tuan.defaultModel.itemPriceResultDO.priceInfo.def; 
                if(def && def.promotionList ){
...
var down = function(){
		...
		if (price == 0) {
			// 处理拍卖
			if(window.TAOBAO_PAIMAI_BIDSAU !="undefined") {console.info("detail脚本执行结束（拍卖）");return;}
			winAlert('\u4EF7\u683C\u5C55\u793A\u4E3A\u7A7A!!');
